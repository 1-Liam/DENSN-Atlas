"""Real-world external evaluation on etcd/raft current-term commit safety."""
...
FIXTURES_DIR = ROOT / "fixtures" / "etcd_raft_current_term"
MANIFEST_PATH = FIXTURES_DIR / "train" / "manifest.json"
VERIFIER_SCRIPT = ROOT / "verifiers" / "etcd_raft_current_term_verifier.py"
CLAIM_KIND = "etcd_raft_current_term_invariant"
...
def real_world_config() -> DENSNConfig:
    return DENSNConfig(
        ...
        candidate_labels=["CurrentTermCommitReady", "RaftCurrentTermCommitReady"],
    )
...
def run_real_world_etcd_raft_benchmark(output_dir: str = "artifacts/real_world") -> dict[str, Any]:
    ...
    train_system = DENSNSystem(
        _make_graph("ETCD_RAFT_REAL"), real_world_config(), registry=registry
    )
    ...
    baseline_no_tsl_system = DENSNSystem(
        _make_graph("ETCD_RAFT_BASELINE_NO_TSL"), no_tsl_config(), registry=OntologyRegistry()
    )
    baseline_no_tsl_summary = baseline_no_tsl_system.run_until_stable()
    baseline_no_conflict_system = DENSNSystem(
        _make_graph("ETCD_RAFT_BASELINE_NO_CONFLICT"),
        no_conflict_memory_config(),
        registry=OntologyRegistry(),
    )
    baseline_no_conflict_summary = baseline_no_conflict_system.run_until_stable()

    graph_path = target_dir / "etcd_raft_current_term_graph.json"
    train_system.graph.save(str(graph_path))
    telemetry_path = target_dir / "etcd_raft_current_term_telemetry.jsonl"
    train_system.telemetry.flush(str(telemetry_path))
    registry_path = target_dir / "etcd_raft_current_term_registry.json"
    ...
    summary = {
        ...
        "domain": "etcd_raft_current_term_real_world",
    ...
    write_json_artifact(
        target_dir / "etcd_raft_current_term_summary.json", summary, version=version
    )
    return summary

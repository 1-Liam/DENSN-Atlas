"""Real-world external evaluation on redsync mutex-extend ownership safety."""
...
FIXTURES_DIR = ROOT / "fixtures" / "redsync_mutex_extend"
MANIFEST_PATH = FIXTURES_DIR / "train" / "manifest.json"
VERIFIER_SCRIPT = ROOT / "verifiers" / "redsync_mutex_extend_verifier.py"
CLAIM_KIND = "redsync_mutex_extend_invariant"
...
def real_world_config() -> DENSNConfig:
    return DENSNConfig(
        ...
        candidate_labels=["ExtendWindowReady", "RedsyncExtendWindow"],
    )
...
def run_real_world_redsync_benchmark(output_dir: str = "artifacts/real_world") -> dict[str, Any]:
    ...
    train_system = DENSNSystem(_make_graph("REDSYNC_REAL"), real_world_config(), registry=registry)
    ...
    baseline_no_tsl_system = DENSNSystem(
        _make_graph("REDSYNC_BASELINE_NO_TSL"), no_tsl_config(), registry=OntologyRegistry()
    )
    baseline_no_tsl_summary = baseline_no_tsl_system.run_until_stable()
    baseline_no_conflict_system = DENSNSystem(
        _make_graph("REDSYNC_BASELINE_NO_CONFLICT"),
        no_conflict_memory_config(),
        registry=OntologyRegistry(),
    )
    baseline_no_conflict_summary = baseline_no_conflict_system.run_until_stable()

    graph_path = target_dir / "redsync_mutex_extend_graph.json"
    train_system.graph.save(str(graph_path))
    telemetry_path = target_dir / "redsync_mutex_extend_telemetry.jsonl"
    train_system.telemetry.flush(str(telemetry_path))
    registry_path = target_dir / "redsync_mutex_extend_registry.json"
    ...
    summary = {
        ...
        "domain": "redsync_mutex_extend_real_world",
    ...
    write_json_artifact(target_dir / "redsync_mutex_extend_summary.json", summary, version=version)
    return summary

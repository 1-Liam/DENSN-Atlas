"""Real-world external evaluation on raft-rs new-leader read-index current-term safety."""
...
FIXTURES_DIR = ROOT / "fixtures" / "raft_rs_read_index_current_term"
MANIFEST_PATH = FIXTURES_DIR / "train" / "manifest.json"
VERIFIER_SCRIPT = ROOT / "verifiers" / "raft_rs_read_index_current_term_verifier.py"
CLAIM_KIND = "raft_rs_read_index_current_term_invariant"
...
def real_world_config() -> DENSNConfig:
    return DENSNConfig(
        ...
        candidate_labels=["CurrentTermReadIndexReady", "RaftRsReadIndexReady"],
    )
...
def _make_graph(prefix: str) -> Any:
    return build_commit_family_graph_from_manifest(MANIFEST_PATH, prefix=prefix)
...
def run_real_world_raft_rs_benchmark(output_dir: str = "artifacts/real_world") -> dict[str, Any]:
    ...
    train_system = DENSNSystem(_make_graph("RAFT_RS_REAL"), real_world_config(), registry=registry)
    ...
    baseline_no_tsl_system = DENSNSystem(
        _make_graph("RAFT_RS_BASELINE_NO_TSL"), no_tsl_config(), registry=OntologyRegistry()
    )
    baseline_no_tsl_summary = baseline_no_tsl_system.run_until_stable()
    baseline_no_conflict_system = DENSNSystem(
        _make_graph("RAFT_RS_BASELINE_NO_CONFLICT"),
        no_conflict_memory_config(),
        registry=OntologyRegistry(),
    )
    baseline_no_conflict_summary = baseline_no_conflict_system.run_until_stable()

    graph_path = target_dir / "raft_rs_read_index_current_term_graph.json"
    train_system.graph.save(str(graph_path))
    telemetry_path = target_dir / "raft_rs_read_index_current_term_telemetry.jsonl"
    train_system.telemetry.flush(str(telemetry_path))
    registry_path = target_dir / "raft_rs_read_index_current_term_registry.json"
    ...
    summary = {
        ...
        "domain": "raft_rs_read_index_current_term_real_world",
    ...
    write_json_artifact(
        target_dir / "raft_rs_read_index_current_term_summary.json", summary, version=version
    )
    return summary

"""Real-world transfer into and out of the external redsync window family."""
...
from .remap_transfer import build_lease_lock_graph_from_manifest
...
PROTOCOL_SUMMARY_PATH = ROOT / "artifacts" / "phase1" / "formal_summary.json"
...
LEASE_MANIFEST_PATH = ROOT / "fixtures" / "lease_lock" / "target" / "manifest.json"
...
def _run_transfer_case(
    *,
    ...
    reuse_system = DENSNSystem(
        graph_builder(target_manifest_path, prefix=f"{target_manifest['task_id'].upper()}_REUSE"),
        reuse_only_config(),
...
def run_real_world_window_transfer(output_dir: str = "artifacts/real_world") -> dict[str, Any]:
    ...
    rows = [
        _run_transfer_case(
            ...
            source_registry_path=PROTOCOL_REGISTRY_PATH,
            source_summary_path=PROTOCOL_SUMMARY_PATH,
        ...
        _run_transfer_case(
            ...
            target_manifest_path=LEASE_MANIFEST_PATH,
            target_claim_kind=LEASE_CLAIM_KIND,
            target_verifier_script=LEASE_VERIFIER_SCRIPT,
    ...
    summary = {
        ...
        "domain": "real_world_window_transfer",
        "rows": rows,
    ...
    write_json_artifact(
        target_dir / "real_world_window_transfer_summary.json", summary, version=version
    )
    return summary

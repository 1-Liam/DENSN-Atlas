"""External verifier for the real-world raft-rs read-index current-term bundle."""
...
EXPECTED_REPO = "https://github.com/tikv/raft-rs"
EXPECTED_COMMIT = "aafb07c7bab439c6139926a77dfafc5b10e9bc84"
...
def verify_manifest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        return {
            ...
            "artifact_ids": [],
            "verifier_name": "raft_rs_read_index_current_term_verifier",
    ...
    if bool(manifest.get("required_stable", False)) and blanket_roles["stable"] < 1:
        return {
            ...
            "failed": True,
            "counterexample": {
                "reason": "missing_current_term_commit_role",
                "blanket_roles": dict(blanket_roles),
            },
            "artifact_ids": [],
            "verifier_name": "raft_rs_read_index_current_term_verifier",
    ...
    markers = {
        "commit_to_current_term": "commit_to_current_term" in source_text,
        "MessageType::MsgReadIndex": "MessageType::MsgReadIndex" in source_text,
        "test_read_only_for_new_leader": "test_read_only_for_new_leader" in source_text,
    ...
    commit_token = str(roles.get("accept_read_index", "ACCEPT_READ_INDEX"))
    clear_token = str(roles.get("read_ctx_ready", "READ_CTX_READY"))
    stable_token = str(roles.get("current_term_committed", "CURRENT_TERM_COMMITTED"))
    counter_prefix = f"{str(roles.get('append_response', 'APPEND_RESPONSE'))}_"
